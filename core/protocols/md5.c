...
void eudora_md5_update(EudoraMD5Context *ctx, const void *data, size_t len) {
    ...
    size_t i = 0;
    if (len >= part) {
        memcpy(&ctx->buffer[index], p, part);
        md5_transform(ctx->state, ctx->buffer);
        for (i = part; i + 63 < len; i += 64)
            md5_transform(ctx->state, &p[i]);
        index = 0;
    }
    memcpy(&ctx->buffer[index], &p[i], len - i);
}
...
void eudora_md5_final(EudoraMD5Context *ctx, unsigned char digest[16]) {
    ...
    int i;
    for (i = 0; i < 8; i++)
        bits[i] = (unsigned char)(ctx->count >> (8 * i));

    const size_t index = (size_t)((ctx->count >> 3) & 0x3F);
    const size_t pad_len = index < 56 ? 56 - index : 120 - index;
    eudora_md5_update(ctx, padding, pad_len);
    eudora_md5_update(ctx, bits, 8);
...
void eudora_md5(const void *data, size_t len, unsigned char digest[16]) {
    ...
    eudora_md5_init(&ctx);
    eudora_md5_update(&ctx, data, len);
...
void eudora_hmac_md5(const void *text, size_t text_len, const void *key,
    ...
    /* Long keys are hashed first (RFC 2104 / md5.c hmac_md5). */
    if (key_len > 64) {
        eudora_md5(key, key_len, tk);
        key = tk;
        key_len = 16;
    }
    ...
    EudoraMD5Context ctx;
    eudora_md5_init(&ctx);
    eudora_md5_update(&ctx, k_ipad, 64);
    eudora_md5_update(&ctx, text, text_len);
    eudora_md5_final(&ctx, digest);

    eudora_md5_init(&ctx);
    eudora_md5_update(&ctx, k_opad, 64);
    eudora_md5_update(&ctx, digest, 16);
    eudora_md5_final(&ctx, digest);
}
